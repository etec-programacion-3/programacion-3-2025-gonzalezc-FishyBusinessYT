...
# Junction table for the many-to-may relationship
# between creatures and passive features:
creatures_passives = db.Table(
    'creatures_passives',
    db.Column('id', db.Integer, primary_key=True),
    db.Column('creature_id', db.Integer, db.ForeignKey(
        'creatures.id'), nullable=False),
    db.Column('passive_id', db.Integer, db.ForeignKey(
        'passives.id'), nullable=False)
    )
...
class Creature(db.Model):
    ...
    stats = db.relationship(
        'Stat', backref='creature',
        lazy=True, cascade='all, delete-orphan'
        )
    actions = db.relationship(
        'Action', backref='creature',
        lazy=True, cascade='all, delete-orphan'
        )
    harvestables = db.relationship(
        'Harvestable', backref='creature',
        lazy=True, cascade='all, delete-orphan'
        )
    passives = db.relationship(
        'Passive', secondary='creatures_passives',
        backref='creatures'
        )
...
class Stat(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    creature_id = db.Column(db.Integer, db.ForeignKey(
        'creatures.id'), nullable=False)
